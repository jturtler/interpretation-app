...
.subscribe(({ data: [model, searchData, page], complete, error }) => {
    getD2().then(d2 => {
        let url = `interpretations?fields=id,type,text,created,lastUpdated,userGroupAccesses[*],externalAccess,publicAccess,likes,likedBy[id,name],user[id,name],comments[id,created,latestUpdate,text,user[id,name]],eventReport[id,name,relativePeriods,userGroupAccesses[*],externalAccess,publicAccess,user[id,name]],eventChart[id,name,relativePeriods,userGroupAccesses[*],externalAccess,publicAccess,user[id,name]],chart[id,name,relativePeriods,userGroupAccesses[*],externalAccess,publicAccess,user[id,name]],map[id,name,mapViews[relativePeriods],userGroupAccesses[*],externalAccess,publicAccess,user[id,name]],reportTable[id,name,relativePeriods,userGroupAccesses[*],externalAccess,publicAccess,user[id,name]]${searchData}`;
        if (page !== undefined) {
